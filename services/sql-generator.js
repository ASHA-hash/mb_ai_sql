...
function buildPrompt(question, context) {
  ...
  return [
    ...
    "- Default period when not specified AND the user asks for a simple summary (no ranking): This Month.",
    "- If no date range is specified and the user is asking for rankings ('top N', 'highest', 'best', 'all time', 'overall') — do NOT add a date filter. Run the query over all historical data.",
    "- If no date range is specified and no ranking is implied: default to current month using MONTH(DateColumn)=MONTH(GETDATE()) AND YEAR(DateColumn)=YEAR(GETDATE()).",
